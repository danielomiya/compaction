...
int main(int argc, char **argv) {
    ...
    if (str_eq(command, PROG_COMMAND)) output = prog(input);
    else if (str_eq(command, PROG2_COMMAND)) output = prog2(input);
    else if (str_eq(command, DESPROG_COMMAND)) output = desprog(input);
    else output = not_found(executable);
